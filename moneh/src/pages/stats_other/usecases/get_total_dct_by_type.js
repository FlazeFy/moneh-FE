...
export default function GetTotalDctByType({ctx}) {
    //Initial variable
    const [error, setError] = useState(null)
    const [isLoaded, setIsLoaded] = useState(false)
    const [items, setItems] = useState([])
    const filter_name = "Dictionaries_Type"
    ...
    const fetchTotalDctByType = () => {
        fetch(`http://127.0.0.1:1323/api/v1/stats/dcttype/desc`)
        .then(res => {
            if(!res.ok){
    ...
    if (error) {
        ...
        return (
            <> 
                ...
                {
                    items ? 
                        <MoleculesChartPie items={items} filter_name={null} is_slug={true}/>  
                    :
