...
export default function GetTotalWishlistByPriority({ctx}) {
    //Initial variable
    const [error, setError] = useState(null)
    const [isLoaded, setIsLoaded] = useState(false)
    const [items, setItems] = useState([])
    const filter_name = "Wishlist_Priority"
    ...
    useEffect(() => {
        fetchTotalWishlistByPriority()
    },[])

    const fetchTotalWishlistByPriority = () => {
        fetch(`http://127.0.0.1:1323/api/v1/stats/wishlistpriority/desc`)
        .then(res => {
            if(!res.ok){
